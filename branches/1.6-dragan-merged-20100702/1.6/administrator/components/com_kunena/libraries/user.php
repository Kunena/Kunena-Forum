...
//FB::log($userids, 'Need to preload uerlist for avatars');
	...
	public function profileIcon($name) {
		switch ($name) {
			case 'gender' :
				...
				return '<span class="sprite-' . $gender . '" title="' . $title . '"></span>';
				break;
			case 'birthdate' :
				if ($this->birthdate)
					return '<span class="sprite-birthdate" title="' . JText::_ ( 'COM_KUNENA_MYPROFILE_BIRTHDATE' ) . ': ' . CKunenaTimeformat::showDate ( $this->birthdate, 'date', 'utc', 0 ) . '"></span>';
				break;
			case 'location' :
				if ($this->location)
					return '<span class="sprite-location" title="' . JText::_ ( 'COM_KUNENA_MYPROFILE_LOCATION' ) . ': ' . kunena_htmlspecialchars ( $this->location ) . '"></span>';
				break;
			case 'website' :
				...
				if ($this->websiteurl)
					return '<a href="' . kunena_htmlspecialchars ( $url ) . '" target="_blank"><span class="sprite-website" title="' . JText::_ ( 'COM_KUNENA_MYPROFILE_WEBSITE' ) . ': ' . kunena_htmlspecialchars ( $websitename ) . '"></span></a>';
				break;
			case 'private' :
	...
	public function socialButton($name, $gray = false) {
		...
		if ($social [$name] ['nourl'] == '0') {
			if (! empty ( $this->$item ))
				return '<a href="' . kunena_htmlspecialchars ( $url ) . '" class="kTip" target="_blank" title="' . $title . ': ' . $value . '"><span class="sprite-' . $name . '"></span></a>';
		} else {
			if (! empty ( $this->$item ))
				return '<span class="sprite-' . $name . ' kTip" title="' . $title . ': ' . $value . '"></span>';
		}
		if ($gray)
			return '<span class="sprite-z_' . $name . '_off"></span>';
		else
			return '';
	}
}
