...
#'global\s*\$database;':'$database = &JFactory::getDBO();',
#'(\s*)(global )\s*\$database,\s*(.*?;)':'\\1\\2\\3\n\\1$database = &JFactory::getDBO();',
#'(\s*)(global .*?),\s*\$database;':'\\1\\2;\n\\1$database = &JFactory::getDBO();',
'\$mosConfig_absolute_path':'JPATH_ROOT',
'\$mosConfig_live_site':'JURI::root()',
'\$mainframe->getCfg\(\'live_site\'\)':'JURI::root()',
'\$mainframe->addCustomHeadTag':'$document->addCustomTag',
'\$database->loadObject\(\$(\w*)\);':'$\\1 = $database->loadObject();',
#'\$(\w+)\s*=\s*\$database->loadObject\(\);':'$database->loadObject($\\1);',
