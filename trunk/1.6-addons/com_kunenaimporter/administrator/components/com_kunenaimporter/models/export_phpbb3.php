...
class KunenaimporterModelExport_phpBB3 extends KunenaimporterModelExport {
	...
	function &exportCategories($start = 0, $limit = 0) {
		$query = "SELECT " . "forum_id AS id, " . "parent_id AS parent, " . "forum_name AS name, " . //			"xxx AS cat_emoticon, ".
		"(forum_status=1) AS locked, " . //			"xxx AS alert_admin, ".
		//			"xxx AS moderated, ".
		//			"xxx AS moderators, ".
		"0 AS pub_access," . "0 AS pub_recurse, " . "0 AS admin_access," . "0 AS admin_recurse, " . "left_id AS ordering, " . "1 AS published, " . //			"xxx AS checked_out, ".
		//			"xxx AS checked_out_time, ".
		//			"xxx AS review, ".
		//			"xxx AS hits, ".
		"forum_desc AS description, " . //			"xxx AS headerdesc, ".
		//			"xxx AS class_sfx, ".
		"forum_last_post_id AS id_last_msg, " . "forum_posts AS numPosts, " . "forum_topics AS numTopics, " . "forum_last_post_time AS time_last_msg " . "FROM #__forums ORDER BY id";
	...
	function &exportMessages($start = 0, $limit = 0) {
		$query = "SELECT " . "p.post_id AS id, " . "IF(p.post_id=t.topic_first_post_id,0,t.topic_first_post_id) AS parent, " . "t.topic_first_post_id AS thread, " . "t.forum_id AS catid, " . "IF(p.post_username, p.post_username, u.username) AS name, " . "p.poster_id AS userid, " . "u.user_email AS email, " . "IF(p.post_subject, p.post_subject, t.topic_title) AS subject, " . "p.post_time AS time, " . "p.poster_ip AS ip, " . //			"xxx AS topic_emoticon, ".
		"(t.topic_status=1 AND p.post_id=t.topic_first_post_id) AS locked, " . "((p.post_approved+1)%2) AS hold, " . "(t.topic_type>0 AND p.post_id=t.topic_first_post_id) AS ordering, " . "t.topic_views AS hits, " . //			"t.topic_moved_id AS moved, ".
		"p.post_edit_user AS modified_by, " . "p.post_edit_time AS modified_time, " . "p.post_edit_reason AS modified_reason, " . "p.post_text AS message " . " FROM `#__posts` AS p " . " LEFT JOIN `#__topics` AS t ON p.topic_id = t.topic_id " . " LEFT JOIN `#__users` AS u ON p.poster_id = u.user_id " . " ORDER BY p.post_id";
		$result = $this->getExportData ( $query, $start, $limit, 'id' );
	...
	function &exportSessions($start = 0, $limit = 0) {
		...
		"user_lastmark AS lasttime, " . //			"readtopics, ".
		"user_lastvisit AS currvisit " . "FROM `#__users` WHERE user_lastvisit>0";
	...
	function &exportUserProfile($start = 0, $limit = 0) {
		$query = "SELECT " . "user_id AS userid, " . //			"xxx AS view, ".
		"user_sig AS signature, " . //			"xxx AS moderator, ".
		//			"xxx AS ordering, ".
		"user_posts AS posts, " . //			"xxx AS avatar, ".
		//			"xxx AS karma, ".
		//			"xxx AS karma_time, ".
		//			"xxx AS group_id, ".
		//			"xxx AS uhits, ".
		...
		"user_from AS location, " . "user_icq AS ICQ, " . "user_aim AS AIM, " . "user_yim AS YIM, " . "user_msnm AS MSN, " . //			"xxx AS SKYPE, ".
		//			"xxx AS GTALK, ".
		//			"xxx AS websitename, ".
		"user_website AS websiteurl, " . //			"xxx AS rank, ".
		//			"xxx AS hideEmail, ".
		//			"xxx AS showOnline ".
		"0 AS dummy " . "FROM `#__users` WHERE user_id > 0 AND user_type != 2 ORDER BY user_id";
