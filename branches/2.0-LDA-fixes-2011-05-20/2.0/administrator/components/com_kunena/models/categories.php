		...
		if ( $this->_admincategories === false ) {
			...
			foreach ($this->_admincategories as $category) {
				...
				if ($category->accesstype != 'none') {
					...
					$kacl = new KunenaAccessJoomla16();
					$category->pub_group = $category->pub_access ? JText::_( $kacl->getGroupName($category->pub_access) ) : JText::_('COM_KUNENA_NOBODY');
				}
				if ($category->accesstype != 'none') {
					...
					if ($jversion->RELEASE == '1.5'){
						...
						$category->admin_group = JText::_ ( $kacl->getGroupName($category->admin_access ));	
					}
