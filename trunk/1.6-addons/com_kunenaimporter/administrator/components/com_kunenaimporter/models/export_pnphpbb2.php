...
class KunenaimporterModelExport_PNphpBB2 extends KunenaimporterModelExport_phpBB2 {
	...
	function checkConfig() {
		...
		if ($version[0] != 2 || $version[1] != 0 || $version[2] < '15') $this->error = "Unsupported forum: phpBB $this->version"; 
		if ($this->error) {
		...
		if (!$this->pnversion) {
			$this->error = $this->ext_database->getErrorMsg();
			if (!$this->error) $this->error = 'Configuration information missing: PNphpBB2 version not found';
		}
		if ($this->error) {
		...
		if ($version[0] != 1 || $version[1] != '2i-p3') $this->error = "Unsupported forum: PNphpBB2 $this->version"; 
		if ($this->error) {
	...
	function &exportUsers($start=0, $limit=0)
	{
		...
		foreach ($result as $item) {
			...
			if ($row->user_regdate > $row->pn_user_regdate) $row->user_regdate = $row->pn_user_regdate;
