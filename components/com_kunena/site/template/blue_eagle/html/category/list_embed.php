							...
							<tr class="k<?php echo $tabclass [$k ^= 1], isset ($category->class_sfx) ? ' k' . $this->escape($tabclass [$k]) . $this->escape($category->class_sfx) : '' ?>"
								...
								<td class="kcol-mid kcol-kcattitle">
									...
									<?php if (!empty($category->description)) : ?>
										<div
											class="kthead-desc km hidden-phone"><?php echo KunenaHtmlParser::parseBBCode($category->description) ?> </div>
									<?php endif; ?>
									<?php
								...
								<td class="kcol-mid kcol-kcattopics hidden-phone">
									<span
										class="kcat-topics-number"><?php echo $this->formatLargeNumber($category->getTopics()) ?></span>
									<span class="kcat-topics"><?php echo JText::_('COM_KUNENA_TOPICS'); ?></span>
								</td>

								<td class="kcol-mid kcol-kcatreplies hidden-phone">
									<span
										class="kcat-replies-number"><?php echo $this->formatLargeNumber($category->getReplies()) ?></span>
								...
								{ ?>
									<td class="kcol-mid kcol-kcatlastpost">
										<?php if ($this->config->avataroncat > 0) : ?>
											...
											if ($useravatar) : ?>
												<span
													class="klatest-avatar hidden-phone"> <?php echo $last->getLastPostAuthor()->getLink($useravatar, null, 'nofollow', '', null, $category->id); ?></span>
											<?php endif; ?>
										<?php endif; ?>
