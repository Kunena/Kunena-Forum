<?php
/**
 * @version $Id: kunena.message.php 1210 2009-11-23 06:51:41Z mahagr $
 * Kunena Component
 * @package Kunena
 *
...
<div align="left" class="kunena-msgtitle"><?php
	...
	<span class="msgkarma">
		<?php if (isset ( $msg_html->karma )) { echo $msg_html->karma;
			if (isset ( $msg_html->karmaplus )) echo '&nbsp;&nbsp;' . $msg_html->karmaplus . ' ' . $msg_html->karmaminus;
			} else { echo '&nbsp;'; } ?>
	</span>
</div>
<div>
	<div class="msgtext">
		<?php echo $msg_html->text; ?>
	</div>
		<?php if (! isset ( $msg_html->closed )) { ?>
	<div id="sc<?php echo $msg_html->id; ?>" class="switchcontent"><!-- make this div distinct from others on this page -->
		<?php //see if we need the users realname or his loginname
			if ($kunena_config->username) { $authorName = $kunena_my->username;
			} else { $authorName = $kunena_my->name; }
		...
		<form action="<?php echo JRoute::_ ( KUNENA_LIVEURLREL . '&amp;func=post' ); ?>" method="post" name="postform" enctype="multipart/form-data">
			...
			<input type="button" class="kbutton kqm_cncl_btn" id="cancel__<?php echo $msg_html->id; ?>" name="cancel" value="<?php @print (_KUNENA_CANCEL) ; ?>" />
...
<div>
	<span><?php if (isset ( $msg_html->signature )) {
