...
class KunenaForumCategoryHelper {
	...
	static public function fixAliases() {
		...
		$rows = 0;
		$queries = array();
		// Fix existing category aliases
		$queries[] = "UPDATE #__kunena_aliases AS a INNER JOIN #__kunena_categories AS c ON a.alias = c.alias SET a.item = c.id WHERE a.type='catid' AND c.alias!=''";
		// Delete non-existing category aliases
