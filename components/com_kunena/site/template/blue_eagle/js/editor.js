...
var kbbcode = new Class({
    ...
    wrapSelection: function(wrapperLeft, wrapperRight, select) {
        ...
        if (!select)
            this.el.selectRange(this.el.getSelectionEnd(), this.el.getSelectionEnd());
        this.selection = this.el.getSelectedRange();
    },
    /*
     Function: insert
...
function kInsertCode() {
    var kcodetype = '';
    if (document.id('kcodetype') != undefined)
        kcodetype = document.id('kcodetype').get('value');
    if (kcodetype != '')
        kcodetype = ' type=' + kcodetype;
...
function newAttachment() {
    if (config_attachment_limit > 0) {
        if (__attachment_limit < config_attachment_limit)
            __attachment_limit++;
        else
            return false;
    }
    ...
    input.addEvent('change', function() {
        ...
        __file.getElement('.kattachment-insert').removeProperty('style').addEvent('click', function() {
            kbbcode.focus().insert('\n[attachment:' + __id + ']' + __filename + '[/attachment]\n', 'after', false);
            return false;
        });
        __file.getElement('.kattachment-remove').removeProperty('style').addEvent('click', function() {
            __file.dispose();
            return false;
        });
        newAttachment();
...
function bindAttachments() {
    ...
    __kattachment.each(function(el) {
        el.getElement('.kattachment-insert').removeProperty('style').addEvent('click', function() {
