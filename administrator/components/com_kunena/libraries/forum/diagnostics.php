...
abstract class KunenaForumDiagnostics {
	...
	public static function getFieldInfo($function) {
		...
		if (!isset($fields[$function])) {
			$fieldsFunction = 'fields_'.$function;
			if (!method_exists(__CLASS__, $fieldsFunction)) $fieldsFunction = 'fields';
			$fields[$function] = self::$fieldsFunction();
		}
	...
	protected static function fields_categoryOrphaned($query = null) {
	...
	protected static function fields_categoryMissingAlias($query = null) {
		if ($query) $query->select('a.id, a.parent_id, a.name, a.alias, a.description');
	...
	protected static function fields_categoryWrongAlias($query = null) {
		if ($query) $query->select('a.id, a.parent_id, a.name, a.alias, a.description');
		return array('name'=>'link', 'alias'=>'invalid', '_link'=>'&view=categories&layout=edit&catid={$id}');
	}
	...
	protected static function fields_attachmentOrphaned($query = null) {
	...
	protected static function fields_pollTopicMismatch($query = null) {
