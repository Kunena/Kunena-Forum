...
<table class="kblocktable<?php echo $htmlClassBlockTable ?>" id="kcat<?php echo intval($section->id) ?>">
	...
	?>
		...
		<?php if (! empty ( $this->pending [$category->id] )) : ?>
			<div class="ks kalert">
				<?php echo JHtml::_('kunenaforum.link', 'index.php?option=com_kunena&view=review&action=list&catid=intval($category->id)', intval($this->pending [$category->id]) . ' ' . JText::_('COM_KUNENA_SHOWCAT_PENDING'), '', '', 'nofollow'); ?>
			</div>
		<?php endif; ?>
			</td>
