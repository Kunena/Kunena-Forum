		...
		if (! isset ( $profiles [$key] )) {
			...
			if ($this->config->showkarma && $this->profile->userid) {
				...
				if ($this->me->userid && $this->me->userid != $this->profile->userid) {
					$this->userkarma_minus = ' ' . JHtml::_('kunenaforum.link', 'index.php?option=com_kunena&view=karma&do=decrease&userid='.$this->profile->userid.'&catid='.$this->topic->category_id.'&pid='.$this->message->id.'&'.JSession::getFormToken().'=1', '<span class="kkarma-minus" alt="Karma-" border="0" title="' . JText::_ ( 'COM_KUNENA_KARMA_SMITE' ) . '"> </span>' );
					$this->userkarma_plus = ' ' . JHtml::_('kunenaforum.link', 'index.php?option=com_kunena&view=karma&do=increase&userid='.$this->profile->userid.'&catid='.$this->topic->category_id.'&pid='.$this->message->id.'&'.JSession::getFormToken().'=1',  '<span class="kkarma-plus" alt="Karma+" border="0" title="' . JText::_ ( 'COM_KUNENA_KARMA_APPLAUD' ) . '"> </span>' );
				}
			}
