...
/**
 * Smilies model for Kunena.
 *
 * @package		Kunena
 * @subpackage	com_kunena
 * @version		1.0
 */
class KunenaModelSmilies extends JXModelList
{
	/**
	...
	function _getListQuery()
	{
		...
		if (!empty($search)) {
			$query->where('a.code LIKE '.$this->_db->Quote('%'.$this->_db->getEscaped($search, true).'%'));
		}
		...
		if ($this->getState('check.state')) {
			...
			if ($state_id !== '*' and $state_id !== null) {
				$query->where('a.palette = '.(int)$state_id);
			}
		}

		// Add the list ordering clause.
		$query->order($this->_db->getEscaped($this->getState('list.ordering', 'a.code')).' '.$this->_db->getEscaped($this->getState('list.direction', 'ASC')));
	...
	function _populateState()
	{
		...
		$this->setState('list.ordering', $app->getUserStateFromRequest($context.'list.ordering', 'filter_order', 'a.code', 'cmd'));
