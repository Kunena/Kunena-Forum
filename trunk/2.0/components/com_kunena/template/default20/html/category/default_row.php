<?php
/**
 * @version $Id$
 * Kunena Component
 * @package Kunena
 *
 * @Copyright (C) 2008 - 2011 Kunena Team. All rights reserved.
 * @license http://www.gnu.org/copyleft/gpl.html GNU/GPL
 * @link http://www.kunena.org
 **/
defined ( '_JEXEC' ) or die ();
?>
					<!-- Loop this LI for each topic in this category -->
					<li class="kcategories-row krow-odd krow-sticky">
						<table summary="List of all forum categories with posts and replies of each">
							<caption>Main Category Title</caption>
							<tbody>
								<tr>
									<td class="ktopic-icon">
										<a href="#" title="View Topic Title"><span class="ktopic-sticky">Sticky</span><!-- <span class="ktopicicon-default">There are no new posts in this category.</span> --></a>
									</td>
									<td class="ktopic-body">
										<ul>
											<li class="ktopic-title">
												<h3><a href="#" title="View Topic Title">Topic Title</a></h3>
												<ul class="ktopic-actions">
													<li><sup class="kindicator-new">(12 NEW)</sup></li>
													<li><span class="kattach-icon">There are attachments in this topic</span></li>
													<li><span class="klocked-icon">This topic is locked</span></li>
													<li><a href="#"><span class="ksubscribe-icon">Subscribe to this topic.</span></a></li>
												</ul>
											</li>
											<li class="ktopic-details">Topic started 2 days, 9 hours ago by <a href="#" title="View Profile">severdia</a></li>
											<li>
												<div class="kpagination-topic">
													<!-- Loop this LI for each page -->
													<ul class="kpage">
														<li class="kpage-title">Page:</li>
														<li><a href="#" title="Page 1 of this topic">1</a></li>
														<li><a href="#" title="Page 2 of this topic">2</a></li>
														<li><a href="#" title="Page 3 of this topic">3</a></li>
														<li class="kpage-more">...</li>
														<li><a href="#" title="Page 14 of this topic">8</a></li>
													</ul>
												</div>
											</li>
											<li>
												<ul class="ktopic-tags">
													<li class="ktopic-tags-title">Topic Tags:</li>
													<li><a href="#">support</a></li>
													<li><a href="#">templates</a></li>
													<li><a href="#">css</a></li>
												</ul>
											</li>
										</ul>
									</td>
									<!-- <td class="kcategory-status-icons"><span class="ktopic-attach">Attachment</span><span class="ktopic-sticky">Sticky</span></td> -->
									<td class="kcategory-replies">22 <span>Replies</span></td>
									<td class="kcategory-views">105k <span>Views</span></td>
									<td class="kcategory-lastpost">
										<ul>
											<li class="kcategory-smavatar"><a href="#" title="View Username's Profile"><img src="images/avatar.png" alt="Username's Avatar" /></a></li>
											<li class="kcategory-smdetails klastpost"><a href="#" title="View Last post title">Last post title</a></li>
											<li class="kcategory-smdetails kauthor">by <a href="#" title="View Author's Profile">Author Name</a></li>
											<li class="kcategory-smdetails kdate">on December 31, 2011</li>
										</ul>
									</td>
									<td class="kcategory-topic-checkbox">
										<input type="checkbox" value="0" name="" class="kmoderate-topic-checkbox">
									</td>
								</tr>
							</tbody>
						</table>
					</li>

					<li class="kcategories-row krow-even">
					<table summary="List of all forum categories with posts and replies of each">
						<caption>Main Category Title</caption>
						<tbody>
							<tr>
								<td class="ktopic-icon">
									<a href="#" title="View Topic Title"><span class="ktopicicon-default">There are no new posts in this category.</span></a>
								</td>
								<td class="ktopic-body">
									<ul>
										<li class="ktopic-title">
											<h3><a href="#" title="View Topic Title">Topic Title</a></h3>
											<ul class="ktopic-actions">
												<li><sup class="kindicator-new">(12 NEW)</sup></li>
												<li><span class="kattach-icon">There are attachments in this topic</span></li>
												<li><span class="klocked-icon">This topic is locked</span></li>
												<li><a href="#"><span class="ksubscribe-icon">Subscribe to this topic.</span></a></li>
											</ul>
										</li>
										<li class="ktopic-details">Topic started 2 days, 9 hours ago by <a href="#" title="View Profile">severdia</a></li>
										<li>
											<div class="kpagination-topic">
												<!-- Loop this LI for each page -->
												<ul class="kpage">
													<li class="kpage-title">Page:</li>
													<li><a href="#" title="Page 1 of this topic">1</a></li>
													<li><a href="#" title="Page 2 of this topic">2</a></li>
													<li><a href="#" title="Page 3 of this topic">3</a></li>
													<li class="kpage-more">...</li>
													<li><a href="#" title="Page 14 of this topic">8</a></li>
												</ul>
											</div>
										</li>
										<li>
											<ul class="ktopic-tags">
												<li class="ktopic-tags-title">Topic Tags:</li>
												<li><a href="#">support</a></li>
												<li><a href="#">templates</a></li>
												<li><a href="#">css</a></li>
											</ul>
										</li>
									</ul>
								</td>
								<!-- <td class="kcategory-status-icons"><span class="ktopic-attach">Attachment</span><span class="ktopic-sticky">Sticky</span></td> -->
								<td class="kcategory-replies">22 <span>Replies</span></td>
								<td class="kcategory-views">105k <span>Views</span></td>
								<td class="kcategory-lastpost">
									<ul>
										<li class="kcategory-smavatar"><a href="#" title="View Username's Profile"><img src="images/avatar.png" alt="Username's Avatar" /></a></li>
										<li class="kcategory-smdetails klastpost"><a href="#" title="View Last post title">Last post title</a></li>
										<li class="kcategory-smdetails kauthor">by <a href="#" title="View Author's Profile">Author Name</a></li>
										<li class="kcategory-smdetails kdate">on December 31, 2011</li>
									</ul>
								</td>
								<td class="kcategory-topic-checkbox">
									<input type="checkbox" value="0" name="" class="kmoderate-topic-checkbox">
								</td>
							</tr>
							</tbody>
						</table>
					</li>