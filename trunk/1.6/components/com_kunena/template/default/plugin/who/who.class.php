...
$kunena_db->setQuery("DELETE FROM #__kunena_whoisonline WHERE time < '{$past}'");
$kunena_db->query();

$kunena_db->setQuery("SELECT COUNT(*) FROM #__kunena_whoisonline WHERE userip='{$myip}' AND userid='{$kunena_my->id}'");
$online = $kunena_db->loadResult();

unset ($row);

if ($task == 'listcat' || $func == 'showcat') {
    $kunena_db->setQuery("SELECT name FROM #__kunena_categories WHERE id='{$catid}'");
    ...
    $kunena_db->setQuery("SELECT subject FROM #__kunena_messages WHERE id='{$id}'");
    $what = $kunena_db->loadResult();
    }
else if ($replyto) {
    $kunena_db->setQuery("SELECT subject FROM #__kunena_messages WHERE id='{$replyto}'");
    $what = $kunena_db->loadResult();
    }
else if ($do == 'reply') {
    $kunena_db->setQuery("SELECT name FROM #__kunena_categories WHERE id='{$catid}'");
    ...
    $kunena_db->setQuery("SELECT name FROM #__kunena_messages WHERE id='{$id}'");
    $what = $kunena_db->loadResult();
    }
else if ($func == 'who') {
...
if ($online == 1) {
    $sql = "UPDATE #__kunena_whoisonline SET time='{$now}', what='{$what}', do= '{$do}', task= '{$task}', link= '{$link}', func= '{$func}'"
