      ...
      <table class="kblock-ban">
        ...
        <tbody>
          ...
          <tr class="krow1">
            ...
            <td class="kcol-mid  kbanfrom"> <span><?php echo $userban->blocked ? JText::_('COM_KUNENA_BAN_BANLEVEL_JOOMLA') : JText::_('COM_KUNENA_BAN_BANLEVEL_KUNENA') ?></span> </td>
            <td class="kcol-mid kbanstart"> <span>
              <?php  if( $userban->created_time ) echo KunenaDate::getInstance($userban->created_time)->toKunena('datetime'); ?>
              </span> </td>
            <td class="kcol-mid kbanexpire"> <span><?php echo $userban->isLifetime() ? JText::_('COM_KUNENA_BAN_LIFETIME') : KunenaDate::getInstance($userban->expiration)->toKunena('datetime'); ?></span> </td>
            <td class="kcol-mid kbancreate"> <span><?php echo $userban->getCreator()->getLink() ?></span> </td>
            <td class="kcol-mid kbanmodify">
              <?php if ( $userban->modified_by && $userban->modified_time) { ?>
              <span> <?php echo $userban->getModifier()->getLink() ?> <?php echo KunenaDate::getInstance($userban->modified_time)->toKunena('datetime'); } ?> </span> </td>
          </tr>
