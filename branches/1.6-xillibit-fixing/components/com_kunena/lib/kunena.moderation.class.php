			...
			case KN_MOVE_MESSAGE : // Move Single message only
				...
				if ( $currentMessage->parent == 0 ) {
					...
					if ( is_object( $newParent ) ) {
						...
						$sql1 = "UPDATE #__fb_messages SET `catid`='$currentMessage->catid', `thread`='$currentMessage->thread', `parent`=0 $subjectupdatesql WHERE `id`='$newParent->id';";
						$this->_db->setQuery ( $sql1 );
						$this->_db->query ();
						$sql2 = "UPDATE #__fb_messages SET `catid`='$currentMessage->catid', `thread`='$currentMessage->thread', `parent`='$currentMessage->thread' $subjectupdatesql WHERE `thread`='{$currentMessage->thread}' AND `id`>'$newParent->id';";
					}
				}
