...
<ul>
	...
	<?php
		if ($params->get ( 'sh_sticky' )) {
			if ($item->ordering) {
				echo '<img src="' . JURI::root () . 'modules/mod_kunenalatest/tmpl/sticky.png" alt="' . JText::_ ( 'KLATESTPOST_STICKY_TOPIC' ) . '" title="' . JText::_ ( 'KLATESTPOST_STICKY_TOPIC' ) . '" />';
			}
		}
	...
	<li><?php
		if (! $params->get ( 'sh_avatar' )) {
			echo JText::_ ( 'LAST_POST_BY' );
			?>&nbsp; <?php
			echo CKunenaLink::GetHrefLink ( JRoute::_ ( "index.php?option=com_kunena&amp;func=profile&amp;Itemid=" . KUNENA_COMPONENT_ITEMID . "&amp;userid=" . $item->userid ), $item->name, '', $rel = 'nofollow', '' );
		}
		?></li>
</ul>
</div>
