...
<?php
        foreach ($rows as $row)
        {
            ...
            $itemlink = $uri->toString(array('scheme', 'host', 'port')) . CKunenaLink::GetThreadPageURL($kunenaConfig, 'view', $row->catid, $row->thread, ceil($row->numberposts / $kunenaConfig->messages_per_page), $kunenaConfig->messages_per_page, $row->lastpostid);
            echo $itemlink;
            echo "</link>\n";
