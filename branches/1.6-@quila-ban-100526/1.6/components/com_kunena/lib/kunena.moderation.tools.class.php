...
class CKunenaModerationTools {
	 ...
	 */
	protected function _banUser($UserID, $expiry, $public_reason, $private_reason, $ip) {
		...
		if ( !$this->_addUserstate( $UserID, KN_USER_BAN, $expiry, $public_reason, $private_reason, $ip ) ) {
			return false;
		}
	 ...
	 * @uses self::_addUserstate()
	 */
	protected function _blockUser($UserID, $expiry, $public_reason, $private_reason, $ip) {
		// sub functions sanitizes input
		...
		if ( !$this->_addUserstate( $UserID, KN_USER_BLOCK, $expiry, $public_reason, $private_reason, $ip ) ) {
			return false;
		}
	 ...
	 */
	private function _addUserstate($UserID, $mode, $expiry, $public_reason, $private_reason, $ip) {
		// Sanitize parameters!
		...
		$query = "INSERT INTO #__kunena_banned_users ( `enabled`, `userid`, `bantype`, `expiry`,  `creation_time`, `created_userid`, `private_reason`,`public_reason`,`ip`)
					VALUES ( 1, {$this->_db->Quote($UserID)}, {$this->_db->Quote($mode)}, {$this->_db->Quote($expiry)}, NOW(), {$this->_db->Quote($this->_my->id)}, {$this->_db->Quote($private_reason)}, {$this->_db->Quote($public_reason)},{$this->_db->Quote($ip)});";
		$this->_db->setQuery ( $query );
		$this->_db->query ();
		check_dberror ( 'Unable to insert user state.' );
...
/**
	 ...
	 *
	 */
	protected function _banIP($UserID,$expiry, $public_reason, $private_reason, $ip) {
		$this->_ResetErrorMessage ();

		if ( !$this->_addUserstate( $UserID, KN_USER_BAN_IP, $expiry, $public_reason, $private_reason, $ip ) ) {
			return false;
		}
	...
	public function blockUser($UserID, $expiry, $public_reason, $private_reason, $ip ) {
		return $this->_blockUser( $UserID, $expiry, $public_reason, $private_reason, $ip );
	}
	...
	public function banUser($UserID, $expiry, $public_reason, $private_reason, $ip ) {
		return $this->_banUser( $UserID, $expiry, $public_reason, $private_reason, $ip );
	}
	...
	public function banIP($UserID,$expiry, $public_reason, $private_reason, $ip) {
		return $this->_banIP($UserID, $expiry, $public_reason, $private_reason, $ip);
	}
