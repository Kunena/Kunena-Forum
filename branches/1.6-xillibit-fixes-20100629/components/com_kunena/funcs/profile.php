...
class CKunenaProfile {
	...
	*/
	...
	protected function saveProfile() {
		...
		$this->_db->setQuery ( "UPDATE {$this->_db->nameQuote('#__kunena_users')} SET personalText={$this->_db->Quote($personnaltext)},birthdate={$this->_db->Quote($birthdate)},
	...
	protected function saveAvatar() {
		...
		if ( $upload->uploaded('avatarfile') ) {
			...
			if ($fileinfo['ready'] === true) {
				...
				$this->_db->setQuery ( "UPDATE {$this->_db->nameQuote('#__kunena_users')} SET avatar={$this->_db->quote('users/'.$fileinfo['name'])} WHERE userid='{$this->profile->userid}'" );
			...
			//set default avatar
			$this->_db->setQuery ( "UPDATE {$this->_db->nameQuote('#__kunena_users')} SET avatar='' WHERE userid={$this->_db->Quote($this->profile->userid)}" );
			$this->_db->query ();
			if (KunenaError::checkDatabaseError()) return;
		} else if ( substr($action, 0, 8) == 'gallery/' && strpos($action, '..') === false) {
			$this->_db->setQuery ( "UPDATE {$this->_db->nameQuote('#__kunena_users')} SET avatar={$this->_db->quote($action)} WHERE userid={$this->_db->Quote($this->profile->userid)}" );
			$this->_db->query ();
			if (KunenaError::checkDatabaseError()) return;
		}
	}
	...
	protected function saveSettings() {
		...
		//Query on kunena user
		$this->_db->setQuery ( "UPDATE {$this->_db->nameQuote('#__kunena_users')} SET ordering={$this->_db->Quote($messageordering)}, hideEmail={$this->_db->Quote($hidemail)}, showOnline={$this->_db->Quote($showonline)}
							WHERE userid={$this->_db->Quote($this->profile->userid)}" );
		$this->_db->query ();
		KunenaError::checkDatabaseError();
	}
	...
	function ban() {
		...
		if (! empty ( $DelAvatar )) {
			...
			$this->_db->setQuery ( "UPDATE {$this->_db->nameQuote('#__kunena_users')} SET avatar=null WHERE userid={$this->_db->Quote($userid)}" );
			$this->_db->Query ();
			KunenaError::checkDatabaseError();
		...
		if (! empty ( $banDelPosts )) {
			...
			$this->_db->setQuery ( "UPDATE {$this->_db->nameQuote('#__kunena_messages')} SET hold=2 WHERE hold!=2 AND userid={$this->_db->Quote($userid)}" );
