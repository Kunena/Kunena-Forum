...
<table class="kblock" id="kforumsearch" >
	...
	<tbody>
		...
		<tr class="krow<?php echo $k ^= 1 . (isset ( $result->class_sfx ) ? ' krow' . $k ^ 1 . $result->class_sfx : '')?>">
			...
			<?php echo CKunenaLink::GetThreadPageLink ( 'view', $result->catid, $result->id, NULL, NULL, $this->escape($result->subject), $result->id )?>
			<br />
			<?php echo $this->escape($result->message)?>
			<br />
			<span style="font-size: x-small;"><?php
				echo JText::_('COM_KUNENA_CATEGORY') . ' ' . CKunenaLink::GetCategoryLink ( 'showcat', $result->catid, $this->escape($result->catname), $rel = 'follow', $class = '', $title = '' )?></span>
			</td>
			<td class="kcol kcol-search-author"><?php echo kunena_htmlspecialchars ( $this->escape($result->name) )?></td>
			<td class="kcol kcol-search-date"><?php echo CKunenaTimeformat::showDate ( $this->escape($result->time) )?></td>
		</tr>
		<?php endforeach; ?>
		...
		<tr class="ksth">
			...
			if ($resStart < $resStop)
				...
				$resStartStop = '0';
			printf ( JText::_('COM_KUNENA_FORUM_SEARCHRESULTS'), $resStartStop, $this->escape($this->total) );
			?>
