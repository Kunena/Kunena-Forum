        ...
        if ($upload->uploaded('avatarfile')) {
            ...
            if (!$fileinfo['status']) {
                ...
                if (!$fileinfo['not_valid_img_ext'])
                    $this->app->enqueueMessage(JText::sprintf('COM_KUNENA_UPLOAD_FAILED', $fileinfo['name']) . ': ' . JText::sprintf('COM_KUNENA_AVATAR_UPLOAD_NOT_VALID_EXTENSIONS', 'gif, jpeg, jpg, png'), 'error');
                else
                    $this->app->enqueueMessage(JText::sprintf('COM_KUNENA_UPLOAD_FAILED', $fileinfo['name']) . ': ' . $fileinfo['error'], 'error');
                return false;
            } else {
