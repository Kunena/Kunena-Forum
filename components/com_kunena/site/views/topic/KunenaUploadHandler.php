 ...
 *
 */
class KunenaUploadHandler
{
	protected $options;

	protected $tmp_upload_path = null;

	protected $tmp_upload_dir = null;
	...
	protected $error_messages = array(
		...
		'max_image_size' => 'Image is too big',
	...
	public function __construct($options = null, $initialize = true, $error_messages = null)
	{
		...
		$this->options = array(
			...
			'upload_dir' => $this->tmp_upload_path,
			...
			'max_file_size' => $config->filesize * 1000,
			'min_file_size' => 1,
			...
			'max_width' => $config->imagewidth,
			'max_height' => $config->imageheight,
			'min_width' => 1,
			'min_height' => 1,
			'max_image_size' => $config->imagesize * 1000,
	...
	protected function generateUploadDir()
	{
		...
		$this->tmp_upload_dir = 'kunena_' . md5($me->id . '/' . $me->username . '/' . $me->name);

		if ( empty($this->tmp_upload_path) )
		{
			$this->tmp_upload_path = KunenaPath::tmpdir() . '/' . $this->tmp_upload_dir . '/';
		}
	}
	...
	protected function checkFileType()
	{
		...
		foreach (glob("*") as $filename) {
			...
			if ( !in_array($mime_type, $list_mime_allwoed) ) return false;
		}
		finfo_close($finfo);
	...
	protected function validate($uploaded_file, $file, $error, $index)
	{
		...
		if (!empty($img_width)) {
			if ($this->options['max_image_size'] && (
					$file_size > $this->options['max_image_size'] ||
					$file->size > $this->options['max_image_size'])
				) {
			$file->error = $this->get_error_message('max_image_size');
			return false;
			}
	...
	protected function handle_file_upload($uploaded_file, $name, $size, $type, $error, $index = null, $content_range = null)
	{
		...
		if ( !empty($file->error) )  $this->last_error = $file->error;
	...
	protected function generate_response($content, $print_response = true)
	{
		if ($print_response) {
			...
			if ( !empty($this->last_error) ) {
				...
				$this->body(json_encode($this->last_error));
			} else {
				$this->body($json);
			}
		}
	...
	public function post($print_response = true)
	{
		...
		return $this->generate_response(
			array($this->options['param_name'] => $files, 'tmp_dir' => $this->tmp_upload_dir, 'file_name' => $files[0]->name ),
			$print_response
		);
	}
	...
	public function delete($print_response = true)
	{
		...
		if ( file_exists($this->tmp_upload_path.'/'.$file_name) )
		{
			unlink($this->tmp_upload_path.'/'.$file_name);
