...
function dofreePDF()
{
	...
	if (in_array($catid, $allow_forum) && $id)
    {
        ...
        $kunena_db->setQuery("SELECT thread FROM {$kunena->nameQuote('#__kunena_messages')} WHERE id={$kunena->Quote($id)} AND catid={$kunena->Quote($catid)}");
        $threadid = $kunena_db->loadResult();
        if (KunenaError::checkDatabaseError()) return;
        //load topic post and details
        $kunena_db->setQuery("SELECT a.*, b.* FROM {$kunena->nameQuote('#__kunena_messages')} AS a, {$kunena->nameQuote('#__kunena_messages_text')} AS b WHERE a.thread={$kunena->Quote($threadid)} AND a.catid={$kunena->Quote($catid)} AND a.parent='0' AND a.id=b.mesid");
		...
		if (empty($row)) { //if the messages doesn't exist don't need to continue
        	...
        	$kunena_db->setQuery("SELECT a.*, b.* FROM {$kunena->nameQuote('#__kunena_messages')} AS a, {$kunena->nameQuote('#__kunena_messages_text')} AS b WHERE a.catid={$kunena->Quote($catid)} AND a.thread={$kunena->Quote($threadid)} AND a.id=b.mesid AND a.parent!='0' ORDER BY a.time ASC");
