			...
			?>
				<tr <?php echo 'class = "row' . $k . '"'; ?>>
					<td align="center"><?php
						...
						?></td>
					<td width="5">
						<input type="radio" id="cb<?php echo kescape($row->directory);?>" name="cid[]" value="<?php echo kescape($row->directory); ?>" onclick="isChecked(this.checked);" />
					</td>
					<td><?php $img_path = JURI::root().'components/com_kunena/template/'.$row->directory.'/images/template_thumbnail.png'; ?>
						...
						echo kescape($row->directory);
						?>','edit')"><?php echo kescape($row->name);?></a>
						</span>
					</td>
					<td align="center">
