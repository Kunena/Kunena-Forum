...
class CKunenaShowcat {
	...
	function __construct($catid, $page=0) {
		...
		$query = "SELECT c.*, s.catid AS subscribeid
				...
				LEFT JOIN {$this->db->nameQuote('#__kunena_subscriptions_categories')} AS s ON c.id = s.catid
				AND s.userid = {$this->db->Quote($this->my->id)}
		...
		$this->db->setQuery ( "SELECT name, id FROM {$this->db->nameQuote('#__kunena_categories')} WHERE id={$this->db->Quote($this->objCatInfo->parent)}" );
		...
		$this->db->setQuery ( "SELECT COUNT(*) FROM {$this->db->nameQuote('#__kunena_messages')} WHERE parent='0' AND catid={$this->db->Quote($this->catid)} AND hold IN ({$hold})" );
		$this->total = ( int ) $this->db->loadResult ();
		KunenaError::checkDatabaseError();
		$this->totalpages = ceil ( $this->total / $threads_per_page );

		$query = "SELECT t.id, MAX(m.id) AS lastid FROM {$this->db->nameQuote('#__kunena_messages')} AS t
	INNER JOIN {$this->db->nameQuote('#__kunena_messages')} AS m ON t.id = m.thread
	...
	FROM (
		...
		FROM {$this->db->nameQuote('#__kunena_messages')} AS m";
			...
			$query .= "
		LEFT JOIN {$this->db->nameQuote('#__kunena_attachments')} AS a ON a.mesid = m.thread
		WHERE m.hold IN ({$hold}) AND m.thread IN ({$idstr})
		GROUP BY thread
	) AS l
	INNER JOIN {$this->db->nameQuote('#__kunena_messages')} AS a ON a.thread = l.thread
	INNER JOIN {$this->db->nameQuote('#__kunena_messages_text')} AS t ON a.thread = t.mesid
	...
	ORDER BY ordering DESC, lastid DESC";
			...
			if ($this->config->shownew && $this->my->id) {
				...
				$this->db->setQuery ( "SELECT thread, MIN(id) AS lastread, SUM(1) AS unread FROM {$this->db->nameQuote('#__kunena_messages')} " . "WHERE hold IN ({$hold}) AND moved='0' AND thread NOT IN ({$readlist}) AND thread IN ({$idstr}) AND time>{$this->db->Quote($this->prevCheck)} GROUP BY thread" );
		...
		$this->db->setQuery ( "SELECT * FROM {$this->db->nameQuote('#__kunena_moderation')} AS m INNER JOIN #__users AS u ON u.id=m.userid WHERE m.catid={$this->db->Quote($this->catid)} AND u.block=0" );
