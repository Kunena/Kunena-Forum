...
class Controller extends BaseController
{
	...
	public function execute($task)
	{
		...
		if ($this->format == 'html')
		{
			if ($content instanceof Exception)
			{
				...
				if (!$this->redirect)
				{
					...
					$this->setRedirect(\Kunena\Forum\Libraries\Route\KunenaRoute::getReferrer());
				}
			}
		...
		$response           = new ResponseJson($content, null, false, !empty($this->redirect));
		...
		if ($content instanceof Exception)
		{
			...
			$exception = new Authorise($content->getMessage(), $content->getCode(), $content);
	...
	public function display($cachable = false, $urlparams = false)
	{
		...
		else
		{
			...
			$routed = $menu->getItem(\Kunena\Forum\Libraries\Route\KunenaRoute::getItemID());
	...
	{
		$this->setRedirect(\Kunena\Forum\Libraries\Route\KunenaRoute::getReferrer($default, $anchor));
	}
}
