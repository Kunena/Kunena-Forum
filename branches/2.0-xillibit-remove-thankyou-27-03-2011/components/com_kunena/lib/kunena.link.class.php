...
class CKunenaLink {
	...
	function GetThankyouLink( $do, $catid, $pid, $targetuserid, $name, $title, $class) {
		$token = '&'.JUtility::getToken().'=1';
		return self::GetSefHrefLink ("index.php?option=com_kunena&view=thankyou&do={$do}&catid={$catid}&pid={$pid}{$token}", $name, $title, 'nofollow', $class);
	}
