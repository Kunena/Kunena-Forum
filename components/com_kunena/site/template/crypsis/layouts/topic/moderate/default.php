...
<div>
  <div class="well" id="kmod-container">
    <form action="<?php echo KunenaRoute::_('index.php?option=com_kunena') ?>" method="post" name="myform" id="myform">
      ...
      <div> <?php echo JText::_('COM_KUNENA_TOPIC'); ?>: <strong><?php echo $this->escape( $this->topic->subject ); ?></strong> </div>
      <div> <?php echo JText::_('COM_KUNENA_CATEGORY'); ?>: <strong><?php echo $this->escape( $this->category->name ) ?></strong> </div>
      ...
      <div>
        ...
        <div> <span title="<?php echo KunenaDate::getInstance($this->message->time)->toKunena('config_post_dateformat_hover'); ?>"> <?php echo JText::_('COM_KUNENA_POSTED_AT')?> <?php echo KunenaDate::getInstance($this->message->time)->toKunena('config_post_dateformat'); ?> </span> <span class="kmessage-by"> <?php echo JText::_('COM_KUNENA_BY') . ' ' . $this->message->getAuthor()->getLink() ?> </span> </div>
        <div><?php echo $this->user->getAvatarImage('', 'list'); ?></div>
        <div><?php echo KunenaHtmlParser::stripBBCode ($this->message->message, 300) ?></div>
      </div>
      <div>
      ...
      <ul>
        <li>
          ...
          <?php echo JText::_ ( 'COM_KUNENA_MODERATION_MOVE_SELECTED' ); ?> </li>
        <li>
          <input id="kmoderate-mode-newer" type="radio" name="mode" value="newer" />
          <?php echo JText::sprintf ( 'COM_KUNENA_MODERATION_MOVE_NEWER', $this->escape($this->replies) ); ?> </li>
      </ul>
      <?php endif; ?>
      <br />
      <?php endif; ?>
      <div><?php echo JText::_ ( 'COM_KUNENA_MODERATION_DEST' );?>:
        <div id="modcategorieslist"> <?php echo JText::_ ( 'COM_KUNENA_MODERATION_DEST_CATEGORY' );?>: <?php echo $this->categorylist ?> </div>
        ...
        <div>
          ...
          <?php echo JText::_ ( 'COM_KUNENA_MODERATION_CHANGE_SUBJECT_ON_REPLIES' ); ?> </div>
        <?php if (!isset($this->message)) : ?>
        <div>
          <input type="checkbox" <?php if ($this->config->boxghostmessage): ?> checked="checked" <?php endif; ?> name="shadow" value="1" />
          <?php echo JText::_ ( 'COM_KUNENA_MODERATION_TOPIC_SHADOW' ); ?> </div>
        <?php endif ?>
      </div>
      <div>
        <input type="submit" class="btn" value="<?php echo JText::_ ( 'COM_KUNENA_POST_MODERATION_PROCEED' ); ?>" />
        <a href="javascript:history.back();" class="btn" ><span><?php echo JText::_ ( 'COM_KUNENA_BACK' ); ?></span></a> </div>
    </form>
  </div>
</div>
