...
{
	$kunena_db->setQuery("SELECT p.userid, p.posts, u.id, u.{$fb_queryName} AS username FROM #__fb_users AS p
						INNER JOIN #__users AS u ON u.id = p.userid
						WHERE p.posts > '0' AND u.block=0 ORDER BY p.posts DESC", 0, $PopUserCount);
	...
	if ( $fbConfig->fb_profile == "jomsocial" ) {
		...
		$kunena_db->setQuery("SELECT a.profileviews AS hits, u.id AS user_id, u.{$fb_queryName} AS user FROM #__alpha_userpoints AS a"
	 	                 ...
	 	                 . " WHERE a.profileviews>'0' ORDER BY a.profileviews DESC", 0, $PopUserCount);
	} else {
