...
<div class="kblock">
	<div class="kheader">
		<h2>
			<span><?php echo !isset($this->message) ? JText::_('COM_KUNENA_TITLE_MODERATE_TOPIC') : JText::_('COM_KUNENA_TITLE_MODERATE_MESSAGE'); ?></span>
		</h2>
	</div>
			...
			<form action="<?php echo KunenaRoute::_('index.php?option=com_kunena') ?>" method="post" name="myform">
					...
					<div class="kmoderate-topicicons">
						<?php foreach ($this->topicIcons as $id => $icon): ?>
							<input type="radio" name="topic_emoticon"
								value="<?php echo $icon->id ?>" <?php echo ($icon->id == $this->topic->icon_id) ? ' checked="checked" ' : '' ?> />
							<img src="<?php echo $this->ktemplate->getTopicIconIndexPath($icon->id, true); ?>" alt=""
								border="0" />
						<?php endforeach; ?>
					</div>
				<?php endif; ?>
				...
				<?php if (isset($this->message)) : ?>
					...
					<div class="kmoderate-message">
						...
						<div
							class="kmessage-msgtext"><?php echo KunenaHtmlParser::stripBBCode($this->message->message, 300) ?></div>
					</div>
					<div>
					...
					<?php if (!empty($this->replies)) : ?>
						<ul>
							<li><input id="kmoderate-mode-selected" type="radio" name="mode" checked="checked"
									value="selected" /><?php echo JText::_('COM_KUNENA_MODERATION_MOVE_SELECTED'); ?>
							</li>
							<li><input id="kmoderate-mode-newer" type="radio" name="mode"
									value="newer"><?php echo JText::sprintf('COM_KUNENA_MODERATION_MOVE_NEWER', $this->escape($this->replies)); ?>
							</li>
						</ul>
					<?php endif; ?>
					<br />
				...
				<div><?php echo JText::_('COM_KUNENA_MODERATION_DEST'); ?>:
					...
					<div id="kmod_subject">
						...
						<input type="text" name="subject"
							value="<?php echo $this->escape(!isset($this->message) ? $this->topic->subject : $this->message->subject); ?>" />
					</div>
					<div>
					...
					<?php if (!isset($this->message)) : ?>
						<div>
							<input
								type="checkbox" <?php if ($this->config->boxghostmessage): ?> checked="checked" <?php endif; ?>
								name="shadow" value="1" />
				...
				<div>
					...
					<a href="javascript:window.history.back();" class="button"><span
							class="kbutton-back"><?php echo JText::_('COM_KUNENA_BACK'); ?></span></a>
				</div>
			</form>
		</div>
	</div>
