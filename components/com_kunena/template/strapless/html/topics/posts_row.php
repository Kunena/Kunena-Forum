...
<tr class="<?php echo $this->getTopicClass('k', 'row') ?>">
  <td class="kcol-first kcol-ktopicicon hidden-phone"> <?php echo $this->getTopicLink ( $this->topic, 'unread', $this->topic->getIcon() ) ?> </td>
...
// FIXME:
	...
	?>
    <div class="ktopic-title-cover"> <?php echo $this->getTopicLink ( $this->topic, $this->message, KunenaHtmlParser::parseText ($this->message->subject, 30), KunenaHtmlParser::stripBBCode ($this->message->message, 500), 'ktopic-title km' ) ?> </div>
  </td>
  <td class="kcol-mid ktopictittle">
    ...
    <div class="ks"> <span class="ktopic-category"> <?php echo JText::sprintf('COM_KUNENA_CATEGORY_X', $this->getCategoryLink ( $this->topic->getCategory() ) ) ?> </span> </div>
  </td>
    ...
    <div class="klatest-post-info">
      <?php
			if ($this->config->avataroncat > 0) :
				...
				if ($useravatar) :
			?>
      <span class="ktopic-latest-post-avatar"> <?php echo $this->message->getAuthor()->getLink( $useravatar ) ?> </span>
      <?php
				endif;
			endif;
			?>
      <span class="ktopic-posted-time" title="<?php echo KunenaDate::getInstance($this->message->time)->toKunena('config_post_dateformat_hover'); ?>"> <?php echo JText::_('COM_KUNENA_POSTED_AT') . ' ' . KunenaDate::getInstance($this->message->time)->toKunena('config_post_dateformat'); ?>&nbsp; </span>
