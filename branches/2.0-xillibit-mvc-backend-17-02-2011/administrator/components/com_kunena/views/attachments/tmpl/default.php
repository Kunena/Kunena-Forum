	...
	<div class="kadmin-functitle icon-files"><?php echo JText::_('COM_KUNENA_ATTACHMENTS_VIEW'); ?></div>
		<form action="<?php echo KunenaRoute::_('index.php?option=com_kunena') ?>" method="post" name="adminForm">
			...
			<tr>
						...
						<input type="text" name="search" id="search" value="<?php echo kescape ($this->state->get('list.search'));?>" class="text_area" onchange="document.adminForm.submit();" />
		...
		<?php
			...
			foreach($this->items as $id=>$attachment) {
				...
				if ( $instance->isImage($attachment->filetype)) list($width, $height) =	getimagesize(JPATH_ROOT.'/'.$attachment->folder.'/'.$attachment->filename);
		?>
