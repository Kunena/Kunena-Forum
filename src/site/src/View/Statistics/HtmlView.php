...
class HtmlView extends KunenaView
{
    ...
    public function displayPostRows()
    {
        ...
        foreach ($this->messages as $message) {
            ...
            $this->cache      = true;
            $this->cachekey   = "{$templateHash}.{$usertype}.t{$this->topic->id}.p{$message->id}";
            ...
            if (!$contents) {
                ...
                $this->postAuthor       = $this->getUserObject((int) $message->userid);
    ...
    public function displayTopicRows()
    {
        ...
        foreach ($this->topics as $this->topic) {
            ...
            // TODO: add context (options, template) to caching
            $this->cache      = true;
            $this->cachekey   = "{$templateHash}.{$usertype}.t{$this->topic->id}.p{$this->topic->last_post_id}";
     ...
     * @throws  Exception
     */
    protected function getUserObject(int $userId)
    {
        if (!array_key_exists($userId, $this->userObjectCache)) {
            $this->userObjectCache[$userId] = KunenaFactory::getUser($userId);
        }

        return $this->userObjectCache[$userId];
    }
    ...
    protected function getAvatarForUser(int $userId): string
    {
        if (!array_key_exists($userId, $this->avatarCache)) {
            $this->avatarCache[$userId] = $this->getUserObject($userId)->getAvatarImage('klist-avatar', 'list');
        }
