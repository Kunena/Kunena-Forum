...
function dofreePDF($kunena_db)
{
    ...
    if (!$is_admin)
    {
        $kunena_db->setQuery("SELECT userid FROM #__kunena_moderation WHERE catid='{$catid}' AND userid='{$kunena_my->id}'");
    ...
    if (!$is_Mod)
    {
        ...
        $kunena_db->setQuery("SELECT id, pub_access, pub_recurse, admin_access, admin_recurse FROM #__kunena_categories WHERE id='{$catid}'");
