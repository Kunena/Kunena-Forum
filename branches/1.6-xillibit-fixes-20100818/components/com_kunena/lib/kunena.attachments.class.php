		...
		foreach ($attachments as $attachment) {
			...
			switch (strtolower($attachment->shorttype)){
				case 'image' :
					...
					$attachment->textLink =CKunenaLink::GetAttachmentLink($this->_escape($attachment->folder),$this->_escape($attachment->filename),$this->_escape($attachment->shortname),$this->_escape($attachment->filename), 'lightbox'.$attachment->mesid.' nofollow').' ('.number_format(intval($attachment->size)/1024,0,'',',').'KB)';
					break;
				default :
					...
					$attachment->textLink = CKunenaLink::GetAttachmentLink($this->_escape($attachment->folder),$this->_escape($attachment->filename),$this->_escape($attachment->shortname),$this->_escape($attachment->filename), 'nofollow').' ('.number_format(intval($attachment->size)/1024,0,'',',').'KB)';
			}
