 ...
 * @version		1.0
 */
class KunenaModelMembers extends JModel
{
	/**
	...
	function getState($property = null)
	{
		if (!$this->__state_set)
		{
			...
			$context	= 'com_kunena.members.';

			// Get the list filters.
			$this->setState('filter.search', JRequest::getString('search', null));
			...
			switch ($params->get('members-order'))
			{
				default:
					$this->setState('list.ordering', 'a.username ASC');
					break;
			}
	...
	function &getItems()
	{
		...
		$key = $this->_getStoreId('kunena.members');
		...
		//$store = $this->_getStoreId('members_list');
	...
	function getTotal()
	{
		...
		// Try to load the value from cache.
		//$cache = &JFactory::getCache('com_kunena', 'output');
		//$store = $this->_getStoreId('members_total');
	...
	function _getListQuery()
	{
		...
		// Resolve foriegn keys with the Kunena_members table.
		$query->select('b.*');
		$query->join('INNER', '#__kunena_members AS b ON a.id = b.user_id');
		...
		// Filter the members over the search string if set.
		$search = $this->getState('filter.search');
		if (!empty($search)) {
			$query->where('a.username LIKE '.$this->_db->Quote('%'.$search.'%'));
		}
