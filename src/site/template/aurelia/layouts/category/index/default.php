...
foreach ($this->sections as $section) :
	...
	<div class="kfrontend shadow-lg rounded border section">
		...
		<h1 class="card-header">
			<?php echo $this->getCategoryLink($section, $this->escape($section->name), null, Template::getInstance()->tooltips(), true, false); ?>
			<small class="hidden-xs-down nowrap" id="ksection-count<?php echo $section->id; ?>">
				<?php echo Category::getInstance()->totalCount($section->getTopics()); ?>
			</small>
		</h1>
					...
					foreach ($this->categories[$section->id] as $category) : ?>
						<tr class="category<?php echo $this->escape($category->class_sfx); ?>"
							...
							<td colspan="6">
								<div>
									<h3>
										<?php echo $this->getCategoryLink($category, $category->name, null, Template::getInstance()->tooltips(), true, false); ?>
										<small class="nowrap">
											<span id="kcatcount"><?php echo Category::getInstance()->totalCount($category->getTopics()); ?></span>
											<span>
												...
												<?php if ($category->locked) : ?>
													<span <?php echo Template::getInstance()->tooltips(true); ?>
															title="<?php echo Text::_('COM_KUNENA_LOCKED_CATEGORY') ?>"><?php echo Icons::lock(); ?></span>
												<?php endif; ?>
									...
									<div>
										<ul class="list-inline">
											...
											<?php foreach ($this->categories[$category->id] as $subcategory) : ?>
												<li>
													...
													<?php if (KunenaConfig::getInstance()->showchildcaticon) : ?>
														...
														<?php echo $this->getCategoryLink($subcategory, '', null, Template::getInstance()->tooltips(), true, false) . '<small class="hidden-xs-down muted"> ('
															. $totaltopics . ')</small>';
													endif;
											...
											<?php if (!empty($this->more[$category->id])) : ?>
												<li>
													<?php echo $this->getCategoryLink($category, Text::_('COM_KUNENA_SEE_MORE'), null, Template::getInstance()->tooltips(), true, false); ?>
								...
								<?php if ($category->getmoderators() && KunenaConfig::getInstance()->listcat_show_moderators) : ?>
									...
									<div class="moderators">
										...
										{
											$modslist[] = KunenaFactory::getUser($moderator)->getLink(null, null, '', null, Template::getInstance()->tooltips());
										}
									...
									<div class="row">
										...
										<div class="col-xs-6 col-md-9" id="kcat-last">
											...
											<div class="col-md-12" id="kcat-last">
												<?php endif; ?>
												<span class="lastpostlink"><?php echo $this->getLastPostLink($category, null, null, Template::getInstance()->tooltips(), 30, false, true) ?></span>
												<br>
