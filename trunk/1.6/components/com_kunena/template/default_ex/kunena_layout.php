...
function KUNENA_get_pathway(&$kunena_db, $obj_kunena_cat, $bool_set_title, $obj_post = 0)
{
    ...
    // Link to forum
    $return .= '&nbsp;'.CKunenaLink::GetCategoryLink('showcat', $obj_kunena_cat->getId(), $obj_kunena_cat->getName());
    ...
    if ($obj_post != 0)
    {
		if ($bool_set_title)
		{
			$document->setTitle($obj_post->subject . ' - ' . stripslashes($kunenaConfig->board_title));
		}
...
function KUNENA_get_menu($cbitemid, $kunenaConfig, $kunenaIcons, $my_id, $type, $view = "", $catid = 0, $id = 0, $thread = 0, $is_moderator = false, $numPending = 0)
{
    ...
    $header = '<div id="kunena_topmenu" ><div id="Kunena_tab"><ul>';

    $header .= ' <li ';
    if ($func == 'latest' || $func == '') $header .= ' class="Kunena_item_active" ';
    $header .=' >'.CKunenaLink::GetShowLatestLink('<span>'.(array_key_exists('showlatest', $kunenaIcons) ? '<img src="' . KUNENA_URLICONSPATH . $kunenaIcons['showlatest'] . '" border="0" alt="' . _KUNENA_ALL_DISCUSSIONS . '" title="' . _KUNENA_ALL_DISCUSSIONS . '"/>' : _KUNENA_ALL_DISCUSSIONS).'</span>');
    $header .= '</li>';
    ...
    if ($my_id != 0)
    {
	    ...
	    $header .=' >'.CKunenaLink::GetShowMyLatestLink('<span>'.(array_key_exists('showmylatest', $kunenaIcons) ? '<img src="' . KUNENA_URLICONSPATH . $kunenaIcons['showmylatest'] . '" border="0" alt="' . _KUNENA_MY_DISCUSSIONS . '" title="' . _KUNENA_MY_DISCUSSIONS . '"/>' : _KUNENA_MY_DISCUSSIONS).'</span>');
	    $header .= '</li>';
    }

    $header .= '<li ';
	 if ($func == 'listcat' ) $header .= ' class="Kunena_item_active" ';
	$header .=' >'.CKunenaLink::GetCategoryListLink('<span>'.(array_key_exists('home', $kunenaIcons) ? '<img src="' . KUNENA_URLICONSPATH . $kunenaIcons['home'] . '" border="0" alt="' . _KUNENA_CATEGORIES . '"  title="' . _KUNENA_CATEGORIES . '" />' : _KUNENA_CATEGORIES).'</span>');
    $header .= '</li>';
    ...
    if ($my_id != 0)
    {
        ...
        $header .=' >'.CKunenaLink::GetMyProfileLink($kunenaConfig, $my_id, '<span>'.(array_key_exists('profile', $kunenaIcons) ? '<img src="' . KUNENA_URLICONSPATH . $kunenaIcons['profile'] . '" border="0" alt="' . _GEN_MYPROFILE . '" title="' . _GEN_MYPROFILE . '"/>' : _GEN_MYPROFILE).'</span>');
        $header .= '</li>';
    }
...
//			}
            if ($is_moderator)
            {
                if ($numPending > 0)
                {
                    ...
                    $header .= '<li>';
                    $header .= CKunenaLink::GetPendingMessagesLink( $catid, '<span>'.(array_key_exists('pendingmessages', $kunenaIcons)
                        ? '<img src="' . KUNENA_URLICONSPATH . $kunenaIcons['pendingmessages'] . '" border="0" alt="' . $numPending . ' ' . _SHOWCAT_PENDING . '" />' : $numcolor . '' . $numPending . '</font> ' . _SHOWCAT_PENDING).'</span>');
                    $header .= '</li>';
                }
            }
            break;
    ...
    if ($kunenaConfig->enablerulespage)
    {
        ...
        $header .= ' >'.CKunenaLink::GetRulesLink($kunenaConfig, '<span>'.(array_key_exists('rules', $kunenaIcons) ? '<img src="' . KUNENA_URLICONSPATH . $kunenaIcons['rules'] . '" border="0" alt="' . _GEN_RULES . '" title="' . _GEN_RULES . '"/>' : _GEN_RULES).'</span>');
        $header .= '</li>';
    }
	if ($kunenaConfig->enablehelppage)
    {
        ...
        $header .= ' >'.CKunenaLink::GetHelpLink($kunenaConfig, '<span>'.(array_key_exists('help', $kunenaIcons) ? '<img src="' . KUNENA_URLICONSPATH . $kunenaIcons['help'] . '" border="0" alt="' . _GEN_HELP . '" title="' . _GEN_HELP . '"/>' : _GEN_HELP).'</span>');
        $header .= '</li>';
	}
    $header .= '</ul></div></div>';
    return $header;
...
function getSearchBox()
{
   ...
   $return .= '<input class="kunena_search_inputbox kunenas" type="text" name="q" size="'. $boxsize . '" value="' . _GEN_SEARCH_BOX . '" onblur="if(this.value==\'\') this.value=\'' . _GEN_SEARCH_BOX . '\';" onfocus="if(this.value==\'' . _GEN_SEARCH_BOX . '\') this.value=\'\';" />';
	$return .= ' <input type="submit" value="'._KUNENA_GO.'" name="submit" class="kunena_button kunenas"/>';
    $return .= '</form></div>';
    return $return;
}
?>
