...
Text::script('COM_KUNENA_EDITOR_POLLS');
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                <button class="btn btn-outline-primary border" data-bs-dismiss="modal" aria-hidden="true"><?php echo Text::_('COM_KUNENA_MODAL_BOX_CATEGORY_TEMPLATE_TEXT_CLOSE'); ?></button>
                <button class="btn btn-outline-primary border" id="modal_confirm_erase"><?php echo Text::_('COM_KUNENA_MODAL_BOX_CATEGORY_TEMPLATE_TEXT_BUTTON_REPLACE'); ?></button>
                <button class="btn btn-outline-primary border" id="modal_confirm_erase_keep_old"><?php echo Text::_('COM_KUNENA_MODAL_BOX_CATEGORY_TEMPLATE_TEXT_BUTTON_REPLACE_KEEP'); ?></button>
            </div>
        </div>
    </div>
</div>
...
<form action="<?php echo KunenaRoute::_('index.php?option=com_kunena') ?>" method="post" class="form-validate" role="form" id="postform" name="postform" enctype="multipart/form-data" data-page-identifier="1">
    ...
    <input id="kurl_topicons_request" type="hidden" value="<?php echo KunenaRoute::_('index.php?option=com_kunena&view=topic&task=topicicons&format=json&' . Session::getFormToken() . '=1', false); ?>" />
    ...
    ?>
        ...
        <input type="hidden" id="ckeditor_dialog_polltitle" name="ckeditor_dialog_polltitle" value="<?php echo $this->message->getTopic()->getPoll()->title ?>" />
        <input type="hidden" id="ckeditor_dialog_polltimetolive" name="ckeditor_dialog_polltimetolive" value="<?php echo $this->message->getTopic()->getPoll()->polltimetolive ?>" />
        <?php foreach ($this->message->getTopic()->getPoll()->getOptions() as $index => $option) : ?>
            <input type="hidden" class="ckeditor_dialog_polloption" id="ckeditor_dialog_polloption<?php echo $index; ?>" name="ckeditor_dialog_polloption<?php echo $index; ?>" value="<?php echo $option->text; ?>" />
        <?php endforeach; ?>
    <?php endif; ?>
    <input type="hidden" id="kunena_upload" name="kunena_upload" value="<?php echo \intval($this->message->catid) ?>" />
    <input type="hidden" id="kunena_upload_files_url" value="<?php echo KunenaRoute::_('index.php?option=com_kunena&view=topic&task=upload&format=json&' . Session::getFormToken() . '=1', false) ?>" />
    ...
    <div class="shadow-lg p-3 mb-5 rounded">
        ...
        <?php if (!$this->category->allowAnonymous && !$this->me->userid) : ?>
            ...
            <div class="form-group row" id="kanynomous-check-name">
                ...
                <div class="col-md-10">
                    <input type="text" id="kauthorname" name="authorname" placeholder="<?php echo Text::_('COM_KUNENA_TOPIC_EDIT_PLACEHOLDER_AUTHORNAME') ?>" class="form-control" maxlength="35" tabindex="4" value="<?php echo $this->escape($this->message->name); ?>" />
        ...
        <?php if ($this->config->askEmail && !$this->me->userid) : ?>
            <div class="form-group row">
                ...
                <div class="col-md-10">
                    <div class="md-form mt-0">
                        <input type="email" id="email" name="email" size="35" placeholder="<?php echo Text::_('COM_KUNENA_TOPIC_EDIT_PLACEHOLDER_EMAIL') ?>" class="form-control validate-email" maxlength="45" tabindex="5" value="<?php echo !empty($this->message->email) ? $this->escape($this->message->email) : '' ?>" required />
                        <br />
        ...
        <div class="form-group row">
            ...
            <div class="col-md-10">
                <?php if (!$this->config->allowChangeSubject && $this->topic->exists() && !KunenaUserHelper::getMyself()->isModerator($this->message->getCategory())) : ?>
                    <input class="form-control" type="text" name="subject" id="subject" value="<?php echo !empty($this->message->subject) ? $this->escape($this->message->subject) : ''; ?>" disabled />
                <?php else : ?>
                    <input class="form-control" type="text" placeholder="<?php echo Text::_('COM_KUNENA_TOPIC_EDIT_PLACEHOLDER_SUBJECT') ?>" name="subject" id="subject" maxlength="<?php echo $this->escape($this->ktemplate->params->get('SubjectLengthMessage')); ?>" tabindex="6" value="<?php echo !empty($this->message->subject) ? $this->escape($this->message->subject) : ''; ?>" />
                <?php endif; ?>
            </div>
        </div>
                            ...
                            <?php foreach ($this->topicIcons as $icon) : ?>
                                ...
                                <?php if ($this->config->topicIcons && $topicicontype == 'svg') : ?>
                                    ...
                                    <?php elseif ($this->config->topicIcons && $topicicontype == 'fa') : ?>
                                        ...
                                        <?php else : ?>
                                            <label class="radio inline" for="radio<?php echo $icon->id; ?>"><img loading=lazy src="<?php echo $icon->relpath; ?>" alt="<?php echo $icon->name; ?>" style="border:0;" />
                                            <?php endif; ?>
                                            </label>
                                        <?php endforeach; ?>
                        </div>
        ...
        <?php echo $this->subLayout('Widget/Editor')
            ...
            ->set('me', $this->me)
            ->set('wa', $this->wa)
            ->set('privateMessage', $this->privateMessage);
        ?>
            ...
            <div class="form-group row" id="kanynomous-check">
                <div class="controls">
                    <div class="custom-control custom-checkbox">
                        <input type="checkbox" class="custom-control-input" id="kanonymous" name="anonymous" value="1" <?php if ($this->postAnonymous) {
                                                                                                                            echo 'checked="checked"';
                                                                                                                        } ?> />
