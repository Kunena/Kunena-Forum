...
class SearchForm extends KunenaLayout
{
	...
	public function displayInput(string $name, string $attributes = '')
	{
		...
		{
			case 'searchatdate':
				return '<input type="text" class="form-control" name="searchatdate" data-date-format="yyy-dd-mm" placeholder="yyy-dd-mm" value="' . $this->state->get('query.searchatdate') . '">' . $attributes;
		}
