<?php
/**
 * @version		$Id: mykunena.php 2857 2010-06-27 12:33:22Z fxstein $
 * @package		Kunena
...
if(!class_exists('plgCommunityMyKunena'))
{
	class plgCommunityMyKunena extends CApplications
	{
		...
		{	
			...
			$css = '';
			if( file_exists(JURI::base() . 'components/com_kunena/template/default/kunena.forum.css')) 
			{
				$css = JURI::base() . 'components/com_kunena/template/default/kunena.forum.css';
			} else {
			...
			$db =& JFactory::getDBO();
			// Get forum user info:
			$db->setQuery("SELECT  a.*, b.* FROM #__kunena_users as a"
						...
						foreach ($items as $item )
						{
							...
							*/
						    ...
						    <li>
						        ...
						        <div class="content">
						            <a href="<?php echo $fbURL;?>" class="kjsubject"><?php echo stripslashes ($item->subject); ?></a> in
									<a href="<?php echo $fbCatURL; ?>" class="kjcategory"><?php echo $item->catname; ?></a>
									on <span class="kjdate"><?php echo $postDate->toFormat(JText::_('DATE_FORMAT_LC2')); ?></span>
						        </div>
						        <div style="clr"></div>
						    </li>
		...
		function getTopicEmoticon(&$item) 
		{
			...
			if( !defined('JB_URLEMOTIONSPATH' )) {
				$emoticonPath = JURI::base() . 'components/com_kunena/template/default/images/emoticons/';
			} else {
