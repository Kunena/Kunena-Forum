...
class KunenaAdminControllerRanks extends KunenaController {
	...
	function save() {
		...
		if (empty ( $rank_title ) || empty ( $rank_image )) {
			$task = ($id == NULL) ? 'add' : 'edit&id=' . $id;
			$app->redirect ( JURI::base () . "index.php?option=$option&task=" . $task, JText::_('COM_KUNENA_MISSING_PARAMETER') );
			$app->close ();
		}

		$db->setQuery ( "SELECT * FROM #__kunena_ranks" );
		...
		if ($id == NULL) {
			...
			$db->setQuery ( "UPDATE #__kunena_ranks SET rank_title = '$rank_title', rank_image = '$rank_image', rank_special = '$rank_special', rank_min = '$rank_min' WHERE rank_id = $id" );
		}
		$db->query ();
		if (KunenaError::checkDatabaseError()) return;
