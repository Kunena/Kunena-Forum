...
<?php
        foreach ($rows as $row)
        {
            ...
            echo "            <link>";
            echo CKunenaLink::GetThreadPageURL($fbConfig, 'view', $row->catid, $row->thread, ceil($row->numberposts / $fbConfig->messages_per_page), $fbConfig->messages_per_page, $row->lastpostid);
            echo "</link>\n";
