...
/**
 * Files model for Kunena.
 *
 * @package		Kunena
 * @subpackage	com_kunena
 * @version		1.0
 */
class KunenaModelFiles extends JXModelList
{
	/**
	...
	function _getListQuery()
	{
		...
		if (!empty($search)) {
			$query->where('a.file_path LIKE '.$this->_db->Quote('%'.$this->_db->getEscaped($search, true).'%'));
		}

		// Add the list ordering clause.
		$query->order($this->_db->getEscaped($this->getState('list.ordering', 'a.file_path')).' '.$this->_db->getEscaped($this->getState('list.direction', 'ASC')));
...
//		$this->setState('filter.state', $app->getUserStateFromRequest($context.'filter.state', 'filter_state', '*', 'string'));
		...
		$this->setState('list.ordering', $app->getUserStateFromRequest($context.'list.ordering', 'filter_order', 'a.file_path', 'cmd'));
