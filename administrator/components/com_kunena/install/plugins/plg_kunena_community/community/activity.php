...
class KunenaActivityCommunity extends KunenaActivity {
	...
	public function onAfterThankyou($target, $actor, $message) {
		...
		$act->target = $target;
		$act->title = JText::_ ( '{single}{actor}{/single}{multiple}{actors}{/multiple} ' . JText::sprintf( 'PLG_KUNENA_COMMUNITY_ACTIVITY_THANKYOU_TITLE', $username ) . '<a href="' . $message->getTopic()->getUrl() . '">' . $message->subject . '</a>' );
		$act->content = NULL;
