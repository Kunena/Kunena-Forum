...
/**
 * Members model for Kunena.
 *
 * @package		Kunena
 * @subpackage	com_kunena
 * @version		1.0
 */
class KunenaModelMembers extends JXModelList
{
	/**
	...
	function _getListQuery()
	{
		...
		if (!empty($search)) {
			$query->where('a.username LIKE '.$this->_db->Quote('%'.$this->_db->getEscaped($search, true).'%'));
		}
		...
		if ($this->getState('check.state')) {
			...
			if ($state_id !== '*' and $state_id !== null) {
				$query->where('a.block = '.(int)$state_id);
			}
		}

		// Resolve foreign keys.
		$query->select('b.*');
		$query->join('LEFT', '`#__kunena_members` AS b ON b.user_id = a.id');
	...
	function _populateState()
	{
		...
		$this->setState('list.ordering', $app->getUserStateFromRequest($context.'list.ordering', 'filter_order', 'a.username', 'cmd'));
