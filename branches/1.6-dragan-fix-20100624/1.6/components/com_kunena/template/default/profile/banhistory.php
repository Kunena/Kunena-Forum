...
<table class="<?php echo isset ( $this->objCatInfo->class_sfx ) ? ' kblocktable' . $this->escape($this->objCatInfo->class_sfx) : ''; ?> kblock-ban">
	...
	<tr class="ksectiontableentry1">
		...
		<td>
			<span><?php  if( $userban->created_time ) echo CKunenaTimeFormat::showDate($this->escape($userban->created_time), 'datetime'); ?> </span>
		</td>
		<td>
		...
		<td>
			<span><?php echo CKunenaLink::GetProfileLink ( $this->escape($userban->created_by) ); ?></span>
		</td>
		<td>
			<?php if ( $userban->modified_by && $userban->modified_time) { ?>
			<span><?php echo CKunenaLink::GetProfileLink ( $this->escape($userban->modified_by) ); ?>  <?php echo CKunenaTimeFormat::showDate($this->escape($userban->modified_time), 'datetime'); } ?></span>
		</td>
	</tr>
	<?php if($userban->reason_public) : ?>
	<tr class="ksectiontableentry2">
		<td colspan="2"><b><?php echo JText::_('COM_KUNENA_BAN_PUBLICREASON'); ?></b> :</td>
		<td colspan="4"><?php echo KunenaParser::parseText ($this->escape($userban->reason_public)); ?></td>
	</tr>
	<?php endif; ?>
	<?php if($userban->reason_private) : ?>
	<tr class="ksectiontableentry2">
		<td colspan="2"><b><?php echo JText::_('COM_KUNENA_BAN_PRIVATEREASON'); ?></b> :</td>
		<td colspan="4"><?php echo KunenaParser::parseText ($this->escape($userban->reason_private)); ?></td>
	</tr>
	<?php endif; ?>
	<?php if (is_array($userban->comments)) foreach ($userban->comments as $comment) : ?>
	<tr class="ksectiontableentry2">
		<td colspan="2"><b><?php echo JText::sprintf('COM_KUNENA_BAN_COMMENT_BY', CKunenaLink::GetProfileLink ( $this->escape($comment->userid) )); ?></b> :</td>
		<td colspan="1"><?php echo CKunenaTimeFormat::showDate($this->escape($comment->time)); ?></td>
		<td colspan="3"><?php echo KunenaParser::parseText ($this->escape($comment->comment)); ?></td>
	</tr>
	<?php endforeach; ?>
	<?php endforeach; ?>
<?php else : ?>
